    ...
    <div class="card shadow mb-4">
        ...
        <div class="card-body">
            <form action="/ubahpeminjam/{{$datapeminjam->id}}" method="POST" enctype="multipart/form-data" >
                ...
                <small>Nama Peminjam</small>
                <input type="text" name="name" value="{{$datapeminjam->name}}" class="form form-control">
                @error('name')
                ...
                <div class="form form-group">
                <small>Email</small>
                <input type="text" name="email" value="{{$datapeminjam->email}}" class="form form-control">
                @error('email')
                    <small>{{ $message }}</small><br>
                @enderror
                </div>
